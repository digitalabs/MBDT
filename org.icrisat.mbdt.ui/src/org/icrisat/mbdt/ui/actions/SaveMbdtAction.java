...
public class SaveMbdtAction implements IWorkbenchWindowActionDelegate {
	
	public static final String ID = "org.icrisat.mbdt.ui.SaveMbdtAction";
		...
		if(saveAsdialog.open()== Window.OK){
			...
			if(Session.getInstance().getRootmodel().getLoadNextGen().isEmpty()){
				...
				try {
					...
					if(subFolder.contains(".mbdt")){
						subFolderSplit = subFolder.substring(0,subFolder.indexOf(".mbdt"));
					}else{
						subFolderSplit = subFolder;
					}
					finalFolder = subFolderSplit +".mbdt";
					if(!new File(finalFolder).exists()){
						 FileOutputStream fos = new FileOutputStream(subFolderSplit+".mbdt");
